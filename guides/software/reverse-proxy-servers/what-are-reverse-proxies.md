...
A reverse proxy can also terminate TLS certificates in one place, apply common security headers, add authentication in front of services that do not have it, and route traffic without exposing every backend service directly to the internet.

Let's say that you have three different services running:

- A Node.js forum, NodeBB, that runs on port `8008`
- A Navidrome music streaming server on port `3000` on a local machine using IP `192.168.1.125`
- An ownCloud web UI on port `8800` on a local machine with an IP of `192.168.1.123`
...
A reverse proxy can be configured to accept requests for this domain and forward them to a different host or port.
...
You don't have to use folders either. You can use subdomains such as `music.mydomain.com`, `cloud.mydomain.com`, and `forum.mydomain.com`, respectively. It's all up to you and how you want to structure your services.
